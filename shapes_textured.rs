...
pub fn draw_triangle_tex(
    ...
    let mut raster_data: HashMap<i32, (i32, i32, f32, f32, f32)> = HashMap::with_capacity(raster_data_size as usize);
    ...
    // Fill the triangle
    for (y, (min_x, max_x, t1x, t2x, ty)) in raster_data {
        let t1 = Vec2::new(t1x, ty);
        let t2 = Vec2::new(t2x, ty);
...
pub fn draw_line_tex(
    ...
    tex_w: u32,
    tex_h: u32,
    mut raster_data: Option<&mut HashMap<i32, (i32, i32, f32, f32, f32)>>,
) {
    ...
    let length_y = (p1 - p2).abs().y;

    for _ in 0..length {
        let cx = i32::abs(current_x - p1.x);
        let cy = i32::abs(current_y - p1.y);
        ...
        if raster_data.is_some() {
            ...
            match raster_data.entry(current_y) {
                ...
                Entry::Vacant(v) => v.insert((current_x, current_x, tex_step_x, tex_step_x, tex_step_y)),
            };
        }
